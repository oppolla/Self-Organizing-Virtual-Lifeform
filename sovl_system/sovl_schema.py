...
class ValidationSchema:
    ...
    def get_schema() -> Dict[str, Dict[str, ConfigSchema]]:
        ...
        return {
            "core_config": {
                "base_model_name": ConfigSchema(field="core_config.base_model_name", type=str, required=True),  # Name of the primary model (e.g., "SmolLM2-360M"), required for model initialization
                ...
                "cross_attn_layers": ConfigSchema(field="core_config.cross_attn_layers", type=list, default=[4, 6]),  # List of layer indices for cross-attention integration; affects model architecture
            ...
            "model": {
                "model_path": ConfigSchema(field="model.model_path", type=str, required=True),  # Path to the primary model's pretrained weights; required for model loading
                "model_type": ConfigSchema(field="model.model_type", type=str, default="causal_lm", validator=lambda x: x in ["causal_lm", "gpt2"]),  # Type of the primary model (e.g., causal_lm for language modeling)
                "quantization_mode": ConfigSchema(field="model.quantization_mode", type=str, default="fp16", validator=lambda x: x in ["fp16", "int8", "none"]),  # Quantization mode for the primary model
            },
            "controls_config": {
                ...
                "scaffold_weight_cap": ConfigSchema(field="controls_config.scaffold_weight_cap", type=float, default=0.5, range=(0.0, 1.0)),  # Maximum weight for scaffold model influence; caps its contribution
                ...
                "blend_strength": ConfigSchema(field="controls_config.blend_strength", type=float, default=0.5, range=(0.0, 1.0)),  # Strength of blending scaffold outputs with primary model
                "attention_weight": ConfigSchema(field="controls_config.attention_weight", type=float, default=0.5, range=(0.0, 1.0)),  # Weight for attention in scaffold integration
                "temp_eager_threshold": ConfigSchema(field="controls_config.temp_eager_threshold", type=float, default=0.7, range=(0.0, 1.0)),  # Threshold for eager temperament state
                "temp_sluggish_threshold": ConfigSchema(field="controls_config.temp_sluggish_threshold", type=float, default=0.3, range=(0.0, 1.0)),  # Threshold for sluggish temperament state
                "temp_mood_influence": ConfigSchema(field="controls_config.temp_mood_influence", type=float, default=0.3, range=(0.0, 1.0)),  # Influence of temperament mood on training
                "temp_curiosity_boost": ConfigSchema(field="controls_config.temp_curiosity_boost", type=float, default=0.2, range=(0.0, 1.0)),  # Boost to curiosity from temperament
                "temp_restless_drop": ConfigSchema(field="controls_config.temp_restless_drop", type=float, default=0.2, range=(0.0, 1.0)),  # Drop in restlessness due to temperament
                "temp_melancholy_noise": ConfigSchema(field="controls_config.temp_melancholy_noise", type=float, default=0.02, range=(0.0, None)),  # Noise scale for melancholy temperament
                "conf_feedback_strength": ConfigSchema(field="controls_config.conf_feedback_strength", type=float, default=0.5, range=(0.0, 1.0)),  # Strength of confidence feedback in temperament
                "temp_smoothing_factor": ConfigSchema(field="controls_config.temp_smoothing_factor", type=float, default=0.5, range=(0.0, 1.0)),  # Smoothing factor for temperament updates
                "temperament_decay_rate": ConfigSchema(field="controls_config.temperament_decay_rate", type=float, default=0.9, range=(0.0, 1.0)),  # Decay rate for temperament state
                "temperament_history_maxlen": ConfigSchema(field="controls_config.temperament_history_maxlen", type=int, default=5, range=(1, None)),  # Maximum length of temperament history
                "confidence_history_maxlen": ConfigSchema(field="controls_config.confidence_history_maxlen", type=int, default=5, range=(1, None)),  # Maximum length of confidence history
                "lifecycle_params": ConfigSchema(field="controls_config.lifecycle_params", type=dict, default={
                    "gestation": {"bias": 0.1, "decay": 1.0},
                    "active": {"bias": 0.0, "decay": 0.9},
                    "sleep": {"bias": -0.1, "decay": 0.8}
                }),  # Lifecycle parameters for temperament in different stages
            },
            "training_config": {
                "model_name": ConfigSchema(field="training_config.model_name", type=str, default="SmolLM2-360M"),  # Name of the model used for training; aligns with core_config.base_model_name
                "learning_rate": ConfigSchema(field="training_config.learning_rate", type=float, default=1.5e-5, range=(0.0, None), required=True),  # Learning rate for optimizer; controls training step size
                "train_epochs": ConfigSchema(field="training_config.train_epochs", type=int, default=1, range=(1, None)),  # Number of training epochs; higher values increase training time
                ...
                "sigmoid_shift": ConfigSchema(field="training_config.sigmoid_shift", type=float, default=3.0, range=(0.0, None)),  # Shift parameter for sigmoid lifecycle curve; affects weight progression
                ...
                "grad_accum_steps": ConfigSchema(field="training_config.grad_accum_steps", type=int, default=4, range=(1, None)),  # Number of gradient accumulation steps; simulates larger batch sizes
                "exposure_gain_eager": ConfigSchema(field="training_config.exposure_gain_eager", type=int, default=2, range=(1, None)),  # Gain for eager exposure in training; boosts early learning for certain samples
                ...
                "warmup_steps": ConfigSchema(field="training_config.warmup_steps", type=int, default=300, range=(0, None)),  # Number of warmup steps; overrides warmup_ratio if set
                "total_steps": ConfigSchema(field="training_config.total_steps", type=int, default=5000, range=(1, None)),  # Total training steps; defines training duration
                ...
                "max_epochs": ConfigSchema(field="training_config.max_epochs", type=int, default=1, range=(1, None)),  # Maximum number of training epochs
                ...
                "enable_gestation": ConfigSchema(field="training_config.enable_gestation", type=bool, default=True),  # If True, enables gestation phase for gradual model warmup
                "enable_sleep_training": ConfigSchema(field="training_config.enable_sleep_training", type=bool, default=True),  # If True, enables sleep training for memory consolidation
                ...
                "dream_memory_weight": ConfigSchema(field="training_config.dream_memory_weight", type=float, default=0.03, range=(0.0, 1.0)),  # Weight for dream memory in training; affects replay influence
                "enable_dreaming": ConfigSchema(field="training_config.enable_dreaming", type=bool, default=True),  # If True, enables dreaming mechanism for synthetic sample generation
                ...
                "dream_noise_scale": ConfigSchema(field="training_config.dream_noise_scale", type=float, default=0.01, range=(0.0, None)),  # Scale of noise added to dream samples; increases variability
                ...
                "dream_temperament_on": ConfigSchema(field="training_config.dream_temperament_on", type=bool, default=False),  # If True, applies temperament influence to dreaming
                ...
                "scaffold_weight": ConfigSchema(field="training_config.scaffold_weight", type=float, default=0.3, range=(0.0, None)),  # Weight for scaffold model in training; balances influence
                "max_dream_memory_mb": ConfigSchema(field="training_config.max_dream_memory_mb", type=int, default=128, range=(1, None)),  # Maximum memory (MB) for dream storage
                "dream_memory_maxlen": ConfigSchema(field="training_config.dream_memory_maxlen", type=int, default=3, range=(1, None)),  # Maximum number of dream samples stored
            },
            "scaffold_config": {
                "model_path": ConfigSchema(field="scaffold_config.model_path", type=str, required=True),  # Path to scaffold model's pretrained weights
                "model_type": ConfigSchema(field="scaffold_config.model_type", type=str, default="gpt2"),  # Type of the scaffold model (e.g., gpt2)
                "tokenizer_path": ConfigSchema(field="scaffold_config.tokenizer_path", type=str, required=True),  # Path to scaffold model's tokenizer
                "quantization_mode": ConfigSchema(field="scaffold_config.quantization_mode", type=str, default="int8", validator=lambda x: x in ["fp16", "int8", "none"]),  # Quantization mode for scaffold model
            },
            "dynamic_weighting": {
                ...
                "history_size": ConfigSchema(field="dynamic_weighting.history_size", type=int, default=5, range=(1, None)),  # Size of history for dynamic weighting calculations
            ...
            "hardware": {
                ...
                "mock_memory_total_mb": ConfigSchema(field="hardware.mock_memory_total_mb", type=float, default=4096.0, range=(0.0, None)),  # Simulated total memory (MB) for testing
                "max_scaffold_memory_mb": ConfigSchema(field="hardware.max_scaffold_memory_mb", type=int, default=128, range=(1, None)),  # Maximum memory (MB) for scaffold storage
            },
            "lora_config": {
            ...
            "curiosity_config": {
                "enable_curiosity": ConfigSchema(field="curiosity_config.enable_curiosity", type=bool, default=True),  # If True, enables curiosity-driven exploration
                "attention_weight": ConfigSchema(field="curiosity_config.attention_weight", type=float, default=0.3, range=(0.0, 1.0)),  # Weight for curiosity-driven attention; balances exploration
                "queue_maxlen": ConfigSchema(field="curiosity_config.queue_maxlen", type=int, default=50, range=(1, None)),  # Maximum length of curiosity question queue
                ...
                "decay_rate": ConfigSchema(field="curiosity_config.decay_rate", type=float, default=0.95, range=(0.0, 1.0)),  # Decay rate for curiosity novelty; controls forgetting
                "question_timeout": ConfigSchema(field="curiosity_config.question_timeout", type=float, default=1800.0, range=(0.0, None)),  # Timeout (seconds) for curiosity questions
                "novelty_threshold_spontaneous": ConfigSchema(field="curiosity_config.novelty_threshold_spontaneous", type=float, default=0.8, range=(0.0, 1.0)),  # Threshold for spontaneous curiosity triggers
                ...
                "pressure_threshold": ConfigSchema(field="curiosity_config.pressure_threshold", type=float, default=0.55, range=(0.0, 1.0)),  # Threshold for curiosity pressure; triggers exploration
                ...
                "base_temperature": ConfigSchema(field="curiosity_config.base_temperature", type=float, default=0.9, range=(0.0, None)),  # Base temperature for curiosity sampling; controls randomness
                "temperament_influence": ConfigSchema(field="curiosity_config.temperament_influence", type=float, default=0.3, range=(0.0, 1.0)),  # Influence of temperament on curiosity; adds emotional context
                ...
                "max_memory_mb": ConfigSchema(field="curiosity_config.max_memory_mb", type=float, default=512.0, range=(1.0, None)),  # Maximum memory (MB) for curiosity storage
                "pressure_change_cooldown": ConfigSchema(field="curiosity_config.pressure_change_cooldown", type=float, default=1.0, range=(0.0, None)),  # Cooldown (seconds) for pressure changes
                "min_pressure": ConfigSchema(field="curiosity_config.min_pressure", type=float, default=0.1, range=(0.0, 1.0)),  # Minimum curiosity pressure
                "max_pressure": ConfigSchema(field="curiosity_config.max_pressure", type=float, default=0.9, range=(0.0, 1.0)),  # Maximum curiosity pressure
                "pressure_decay_rate": ConfigSchema(field="curiosity_config.pressure_decay_rate", type=float, default=0.95, range=(0.0, 1.0)),  # Decay rate for curiosity pressure
                "metrics_maxlen": ConfigSchema(field="curiosity_config.metrics_maxlen", type=int, default=1000, range=(1, None)),  # Maximum length of curiosity metrics history
                ...
                }),  # Lifecycle parameters for curiosity in different stages
            },
            "cross_attn_config": {
                "memory_weight": ConfigSchema(field="cross_attn_config.memory_weight", type=float, default=0.2, range=(0.0, 1.0)),  # Weight for memory in cross-attention; balances past and present
            },
            "logging_config": {
            ...
            "error_config": {
                ...
                "warning_threshold": ConfigSchema(field="error_config.warning_threshold", type=float, default=5.0, range=(0.0, None)),  # Threshold for logging warnings; based on error frequency
                "error_threshold": ConfigSchema(field="error_config.error_threshold", type=float, default=7.0, range=(0.0, None)),  # Threshold for logging errors; based on severity
            ...
            "data_config": {
                "batch_size": ConfigSchema(field="data_config.batch_size", type=int, default=2, range=(1, None)),  # Batch size for data loading; affects preprocessing speed
            ...
            "data_provider": {
                "provider_type": ConfigSchema(field="data_provider.provider_type", type=str, default="default"),  # Type of data provider (e.g., default, file, database)
                "data_path": ConfigSchema(field="data_provider.data_path", type=str, required=True),  # Path to training data
            },
            "memory_config": {
            ...
            "state_config": {
                "max_history": ConfigSchema(field="state_config.max_history", type=int, default=100, range=(1, None)),  # Maximum number of state history entries to store
            ...
            "temperament_config": {
                ...
                "temperament_history_maxlen": ConfigSchema(field="controls_config.temperament_history_maxlen", type=int, default=5, range=(3, 10)),
                "confidence_history_maxlen": ConfigSchema(field="controls_config.confidence_history_maxlen", type=int, default=5, range=(3, 10))
                "mood_influence": ConfigSchema(field="temperament_config.mood_influence", type=float, default=0.3, range=(0.0, 1.0)),  # Influence of mood on model behavior; adds emotional context
            ...
            "dream_memory_config": {
                "max_memories": ConfigSchema(field="dream_memory_config.max_memories", type=int, default=100, range=(1, None)),  # Maximum number of dream memories stored
                "base_weight": ConfigSchema(field="dream_memory_config.base_weight", type=float, default=0.1, range=(0.0, 1.0)),  # Base weight for dream memories
                "max_weight": ConfigSchema(field="dream_memory_config.max_weight", type=float, default=1.5, range=(0.0, None)),  # Maximum weight for dream memories
            }
        }
