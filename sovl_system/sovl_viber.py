...
class VibeSculptor:
    ...
    def sculpt_vibe(
        ...
        try:
            ...
            energy_components = self._compute_energy(
                user_input, turn_metadata, repetition_factor, perturbation_scale, seed
            )
            flow_components = self._compute_flow(
                user_input, context_data, turn_metadata, repetition_factor, perturbation_scale, seed
            )
            resonance_components = self._compute_resonance(
                user_input, state, turn_metadata, repetition_factor, perturbation_scale, seed
            )
            engagement_components = self._compute_engagement(
                user_input, engagement_manager, turn_metadata, repetition_factor, perturbation_scale, seed
            )
            ...
            energy_components["base_energy"] *= (
                1 - self.coupling_factor * max(0, flow_components["rhythm_score"] - 0.5)
            )
            flow_components["rhythm_score"] *= (
                1 - self.coupling_factor * max(0, resonance_components["topic_consistency"] - 0.5)
            )
            resonance_components["topic_consistency"] *= (
                1 - self.coupling_factor * max(0, engagement_components["engagement_score"] - 0.5)
            )
            engagement_components["engagement_score"] *= (
                1 - self.coupling_factor * max(0, energy_components["base_energy"] - 0.5)
            )
